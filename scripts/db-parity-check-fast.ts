...
async function run() {
  ...
  const singleSql = `
    SELECT json_build_object(
      ...
      'userParity', (
        SELECT json_build_object(
          'totalLegacy', (SELECT count(*) FROM "users"),
          'missingInTarget', (SELECT count(*) FROM "users" l WHERE NOT EXISTS (SELECT 1 FROM "User" t WHERE t.id = l.id)),
          'allNamesMatch', NOT EXISTS (
            SELECT 1 FROM "users" l 
            JOIN "User" t ON t.id = l.id 
            WHERE t.name <> TRIM(COALESCE(l."firstName", '') || ' ' || COALESCE(l."lastName", ''))
          ),
          'users', (
      ...
      'integrityChecks', (
        SELECT json_build_object(
          ...
          'orphanCdrBillingEvents', (SELECT count(*) FROM "Cdr" c LEFT JOIN "BillingEvent" be ON be.id = c."billingEventId" WHERE be.id IS NULL),
          'orphanWallets', (SELECT count(*) FROM "Wallet" w WHERE w."agentId" IS NULL AND w."clientId" IS NULL AND NOT w."isPlatform"),
          'duplicateUserEmails', (SELECT count(*) FROM (SELECT email FROM "User" GROUP BY email HAVING count(*) > 1) d),
          'duplicateNumbers', (SELECT count(*) FROM (SELECT e164 FROM "Number" GROUP BY e164 HAVING count(*) > 1) d)
        )
      )
    ) as audit_report
  `;
  ...
  try {
    const res: any = await prisma.$queryRawUnsafe(singleSql);
    console.log('AUDIT_REPORT_JSON:' + JSON.stringify(res[0].audit_report));
  } catch (err: any) {
    console.error('Audit query error:', err.message || err);
  } finally {
