...
export const ClientResetPasswordModal: React.FC<ClientResetPasswordModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-[var(--bg-surface)]/60 backdrop-blur-xs p-4 overflow-y-auto">
      <div className="bg-[var(--glass-bg)] backdrop-blur-md border border-[var(--glass-border)] rounded-2xl max-w-md w-full shadow-2xl overflow-hidden my-8">
        {/* Header */}
        <div className="flex items-center justify-between p-6 border-b border-[var(--glass-border)] bg-[var(--glass-bg)]/50">
          <div className="flex items-center gap-3">
            <div className="p-2 bg-amber-500/10 bg-amber-500/20 text-[var(--accent-amber)] rounded-xl">
              <KeyRound className="w-5 h-5" />
            </div>
            <div>
              <h2 className="text-lg font-semibold text-[var(--text-primary)] text-white">Reset Client Password</h2>
        ...
        {newPasswordResult ? (
          <div className="p-6 space-y-4">
            <div className="p-4 bg-[var(--accent-emerald-dim)] border border-emerald-200 border-emerald-800/60 rounded-xl">
              <h3 className="text-sm font-semibold text-emerald-900 text-emerald-200">
                Password Successfully Reset!
              </h3>
              <p className="text-xs text-[var(--accent-emerald)] mt-1">
                Provide the temporary password below to the client. This will revoke existing active sessions.
              </p>
            </div>

            <div className="space-y-2 p-4 bg-[var(--glass-bg)]/50 border border-[var(--glass-border)] rounded-xl">
              <div className="flex items-center justify-between">
                ...
                <button
                  type="button"
                  onClick={handleCopy}
                  className="flex items-center gap-1 text-xs text-indigo-600 text-[var(--accent-violet)] hover:text-indigo-700 font-medium"
                >
                  {copied ? (
                    <>
              ...
              <div className="font-mono text-sm bg-[var(--glass-bg)] backdrop-blur-md p-3 rounded-lg border border-[var(--glass-border)] text-[var(--accent-amber)] select-all break-all">
                {newPasswordResult}
              </div>
            </div>
          ...
          <form onSubmit={handleSubmit} className="p-6 space-y-4">
            ...
            <div className="space-y-3">
              <div className="flex items-center justify-between p-3 bg-[var(--glass-bg)]/40 rounded-xl border border-[var(--glass-border)]">
                <div>
                  <span className="text-xs font-semibold text-[var(--text-primary)] text-white block">Auto-generate Password</span>
                  <span className="text-[11px] text-[var(--text-secondary)]">Generate a cryptographically secure 20-character secret</span>
                </div>
                <input
                  type="checkbox"
                  checked={autoGenerate}
                  onChange={(e) => setAutoGenerate(e.target.checked)}
                  className="rounded border-[var(--glass-border)] text-indigo-600 focus:ring-indigo-500"
                />
              </div>
              ...
              <div className="p-3 bg-[var(--accent-amber-dim)] border border-[rgba(245,158,11,0.2)] rounded-xl text-xs text-[var(--accent-amber)]">
